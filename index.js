...
function getLastCarInfo(inventory) {
  /* code here */
  let newArr = inventory.length - 1
  let make = inventory[newArr].car_make
  let model = inventory[newArr].car_model
  return `This is a ${make} ${model}`
}
...
function sortCarInventory(inventory) {
  return inventory.sort((car1, car2) => {
...
function getModelYears(inventory) {
  const carYear = []
  for (let i = 0; i < inventory.length; i++) {
    const years = inventory[i].car_year
    carYear.push(years)
  }
  return carYear
}
